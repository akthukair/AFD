...
def plot_star(Teff, R, time, flux, n, plot_path, plot_file_name):
    ...
    for i in range(len(flux_difference)):
        if flux_difference[i] > threshold:
            ...
            time_start_value.append(time[i])
    ...
    time_end_value = [time[i] for i in time_end_index]
    ...
    for i in range(len(time_start_index)):
        if time_end_index[i] - max_flare_index[i] >= 2 and \
           ...
           time_end_value[i] - time_start_value[i] >= 0.05 and \
           time_end_value[i] - max_flare_time[i] > max_flare_time[i] - time_start_value[i] and \
    ...
    time_start_value = time_start_value[:len(time_end_value)]
    max_flare_index = max_flare_index[:len(time_end_index)]
    max_flare_time = max_flare_time[:len(time_end_value)]
    ...
    return time_start_index, time_end_index, time_start_value, time_end_value, energy, excluded_indexes, max_flare_index, max_flare_time, flare_amp
...
for file_name in file_list:
    ...
    ##########################################
    try:
        time_start_index, time_end_index, time_start_value, time_end_value, energy, excluded_indexes, max_flare_index, max_flare_time, flare_amp = plot_star(
    ...
    time_start_value_list.extend(time_start_value)
    time_end_value_list.extend(time_end_value)
...
final_data_frame = pandas.DataFrame(
    zip(kepler_id_list, quarter_list, time_start_index_list, time_end_index_list, time_start_value_list,
        time_end_value_list, max_flare_index_list, max_flare_time_list, energy_list, excluded_indexes_list,
        Teff_list,
        Radius_list, logg_list, Prot_list, flare_amp_list),
    columns=['kepler_id', 'quarter', 'time_start_index', 'time_end_index', 'time_start_value', 'time_end_value',
                 'flare_peak_index', 'flare_peak_time', 'energy', 'excluded', 'Teff', 'Radius', 'logg', 'Prot',
                 'flare_amp'])

final_data_frame['time_difference'] = final_data_frame.time_end_value - final_data_frame.time_start_value
final_data_frame['time_start_BJD'] = final_data_frame.time_start_value + 54833
final_data_frame['time_end_BJD'] = final_data_frame.time_end_value + 54833
